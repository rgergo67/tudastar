    ...
    <div class="min-h-screen pt-24 bg-gray-100">
        <div class="max-w-7xl mx-auto">
            <div class="w-full sm:max-w-7xl mt-6 p-6 bg-white shadow-md overflow-hidden sm:rounded-lg prose lg:prose-lg">
                ...
                <p>{{ $firstMeaningfulRow[0] }} és {{ $lastMeaningfulRow[0] }} között a {{ $moneyTypes[request('index1', 1)] }} {{ $firstMeaningfulRow[1] }} Ft-ról {{ $lastMeaningfulRow[1] }} Ft-ra nőtt, azaz {{ round($lastMeaningfulRow[1] / $firstMeaningfulRow[1], 2) }}x több lett. Kezdetben egy "{{ $goods[request('index2', 4)] }}" {{ $firstMeaningfulRow[2] }} Ft-ba került, végül {{ $lastMeaningfulRow[2] }} Ft-ba, ami {{ round($lastMeaningfulRow[2] / $firstMeaningfulRow[2], 2) }}x árváltozás. Régen {{ round($firstMeaningfulRow[3]) }} db jött ki a pénzből, ma {{ round($lastMeaningfulRow[3]) }}, ez {{ round($lastMeaningfulRow[3] / $firstMeaningfulRow[3], 2) }}x @if($lastMeaningfulRow[3] < $firstMeaningfulRow[3]) <span class="text-red-600 font-bold">kevesebb</span> @else <span class="text-emerald-600 font-bold">több</span> @endif .</p>

                <div id="linechart-inflacio" class="w-full mt-6" style="height: 500px"></div>

                <p>A <span class="text-emerald-600 font-bold">zöld vonal</span> mutatja a fizetések emelkedését, a <span class="text-red-600 font-bold">piros</span> az adott termék árának növekedését, a <span class="text-yellow-500 font-bold">sárga</span> pedig azt, hogy adott évben abból a fizetésből mennyit tudtál megvenni.</p>
